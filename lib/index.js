...
var MANAGER_PATH = process.env.PRIVACY_MANAGER || "/root/privacy-gateway/scripts/privacy-manager.py";
  ...
  return new Promise((resolve) => {
    ...
    execFile(
      ...
      (err, stdout) => {
        ...
        resolve({
          ok: false,
          error: err ? `${err.message}` : "manager produced no JSON"
        });
      }
    );
  });
...
// src/protect.ts
import { readFileSync, writeFileSync, copyFileSync, existsSync as existsSync2 } from "node:fs";

// ../.dsh/profiles/web/node_modules/js-yaml/dist/js-yaml.mjs
...
var MANAGED_KEY = "privacy-guard-managed";
var SOURCE_KEY = "privacy-guard-source";
function listProviders() {
  const cfg = readSettings();
  const rows = [];
  for (const [kind, block] of Object.entries(cfg || {})) {
    const providers = block?.providers;
    if (!providers || typeof providers !== "object") continue;
    for (const [name2, provider] of Object.entries(providers)) {
      rows.push({
        name: name2,
        baseURL: String(provider?.baseURL || ""),
        kind,
        managed: Boolean(provider && provider[MANAGED_KEY]),
        source: provider && provider[SOURCE_KEY] ? String(provider[SOURCE_KEY]) : void 0
      });
    }
  }
  return rows;
}
function detectGatewayProviders() {
  const cfg = readSettings();
  const found = [];
  for (const [kind, block] of Object.entries(cfg || {})) {
    const providers = block?.providers;
    ...
    for (const [name2, provider] of Object.entries(providers)) {
      ...
      if (/\b8317\b/.test(base) || /privacy-gateway/i.test(base)) {
        found.push({ name: name2, baseURL: base, kind });
      }
    }
  }
  return found;
...
function readSettings() {
  ...
  if (!existsSync2(file)) throw new Error(`no settings file at ${file}`);
  return yaml.load(readFileSync(file, "utf8"));
}
function applyProtectedProvider(sourceName, suffix, displaySuffix, gatewayBaseUrl, directBaseUrl) {
  const file = settingsPath();
  let cfg;
  try {
    cfg = readSettings();
  ...
  if (!source) {
    if (gatewayProviders.length === 1) source = gatewayProviders[0].name;
      ...
      return {
        ok: false,
        error: `several providers point at the gateway (${gatewayProviders.map((p) => p.name).join(", ")}); name one explicitly`
      };
    }
  }
  let block = null;
  ...
  if (sourceProvider[MANAGED_KEY]) {
    return {
      ok: false,
      error: `${source} is a copy created by this plugin, not an original provider. Choose the provider you want a protected twin of.`,
      settings_file: file
    };
  }
  ...
  if (sourceIsGateway) {
    if (!directBaseUrl) {
      return {
        ok: false,
        error: `provider ${source} still points at the gateway (${sourceBase}) and no direct endpoint is configured, so the copy would be identical to the original. Set the direct backend URL first.`,
        settings_file: file
      };
    }
  ...
  const twinSame = existed && existing && String(existing.baseURL || "") === gatewayBaseUrl && String(existing.displayName || "") === `${baseName}${displaySuffix}` && String(existing[SOURCE_KEY] || "") === source;
  const sourceIsIntended = !sourceIsGateway && (!directBaseUrl || sourceBase === directBaseUrl);
...
function mirrorModels(models, efforts) {
  return models.map((m) => {
    const out = { id: String(m.id) };
    if (m.name !== void 0) out.name = m.name;
    if (typeof m.contextWindow === "number") out.contextWindow = m.contextWindow;
    if (typeof m.maxTokens === "number") out.maxTokens = m.maxTokens;
    const modalities = m.input ?? m.inputModalities;
...
function addGatewayProvider(opts) {
  const file = settingsPath();
  let cfg;
  try {
    cfg = readSettings();
  ...
  if (!wantedModels.length) {
    return {
      ok: false,
      error: "no models to mirror; the source provider reported none (read the resolved settings, not the raw file, so schema defaults are included)",
      settings_file: file
    };
  }
  ...
  const same = existed && existing && String(existing.baseURL || "") === opts.baseURL && String(existing.displayName || "") === opts.displayName && String(existing[MANAGED_KEY] || "") === "true" && JSON.stringify(existing.models ?? null) === JSON.stringify(wantedModels.length ? wantedModels : null) && JSON.stringify(existing.compat ?? null) === JSON.stringify(opts.compat ?? null);
  if (same) {
    return {
      ok: true,
      action: "noop",
...
function removeProtectedProvider(name2) {
  const file = settingsPath();
  let cfg;
  try {
    cfg = readSettings();
  ...
  for (const value of Object.values(cfg || {})) {
    const providers = value?.providers;
    if (providers && providers[name2]) {
      const backup = `${file}.bak-plugin-${Date.now()}`;
      try {
        copyFileSync(file, backup);
        delete providers[name2];
        writeFileSync(file, yaml.dump(cfg, { noRefs: true, lineWidth: 120, quotingType: '"' }), "utf8");
      } catch (e) {
        return { ok: false, error: e?.message || String(e), settings_file: file };
      }
      return { ok: true, action: "updated", provider: name2, settings_file: file, backup };
    }
  }
  return { ok: false, error: `provider ${name2} not found`, settings_file: file };
}
...
function apply(ctx, config) {
      ...
      handler: async (req, res) => {
        try {
          ...
          if (action === "builtin_status") {
            const provider = String(body.provider || "deepseek-official");
            ...
            writeJson(res, 200, {
              ...
              error: fallback ? void 0 : "llm-deepseek is not registered"
            });
            return;
          }
          if (action === "builtin") {
            const name2 = String(body.name || "deepseek-protected");
            const displayName = String(body.display_name || "DeepSeek \u5B98\u65B9(\u51ED\u636E\u4FDD\u62A4)");
            const baseUrl = String(body.base_url || "");
            const apiKeyEnv = String(body.api_key_env || "DEEPSEEK_API_KEY");
            let models = Array.isArray(body.models) ? body.models : [];
            if (!models.length) {
              const catalog = await readBuiltinModels(
                String(body.provider || "deepseek-official")
              );
              if (catalog) models = catalog.models;
            }
            const ctxWindow = Number(body.default_context_window || 0) || void 0;
            const declaredEfforts = Array.from(
              new Set(
                models.flatMap((m) => Array.isArray(m?.effortIds) ? m.effortIds : []).map((x) => String(x))
              )
            );
            const wireFor = (level) => level === "off" ? "none" : level;
            const efforts = body.efforts ?? (declaredEfforts.length ? Object.fromEntries(declaredEfforts.map((lv) => [lv, wireFor(lv)])) : { off: "none", low: "low", high: "high", max: "max" });
            writeJson(res, 200, addGatewayProvider({
              name: name2,
              displayName,
              baseURL: baseUrl,
              apiKeyEnv,
              modelsNs: String(body.models_ns || "llm-pi-ai"),
              models,
          ...
          let directBase = String(body.direct_base_url || "");
          if (!directBase) {
            const st = await runManager(["config"]);
            const backend = String(st?.config?.direct_base_url || "");
  ...
  const settings = ctx.get ? ctx.get("settings") : void 0;
  const resolvedNamespace = (ns) => {
    try {
    ...
    return fromFile.ok ? fromFile.data : void 0;
  };
  const llm = ctx.get ? ctx.get("llm") : void 0;
  const readBuiltinModels = async (provider) => {
    if (!llm || typeof llm.listModels !== "function") return null;
    let list;
    try {
      list = await llm.listModels(provider);
    } catch {
      return null;
    }
    ...
    for (const m of list) {
      ...
      const effortIds = (resolved?.reasoning?.efforts || []).map(
        (e) => String(e?.id ?? e)
      );
      models.push({
        id: String(m.id),
